    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (credentials.email.includes('@')) {
          ...
          if (!credentials.email.endsWith('@uninorte.edu.co')) return null;
        } else {
        ...
        const res = await fetch(
          'https://qualiun.pockethost.io/api/collections/users/auth-with-password',
          {
            method: 'POST',
            headers: {
        ...
        if (res.status === 200) {
          if (!json.record.verified) return null;
